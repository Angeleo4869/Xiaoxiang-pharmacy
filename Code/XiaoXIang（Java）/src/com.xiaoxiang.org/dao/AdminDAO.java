...
public class AdminDAO extends BaseHibernateDAO {
	...
	public void delete(Admin persistentInstance) {
		try {
			session=getSession();
			transation = session.beginTransaction();
			persistentInstance = session.get(Admin.class, persistentInstance.getIdAdmin());
			//session.get
...
/*
	public Admin findById(java.lang.Integer id) {
		...
		try {
			Admin instance = (Admin) getSession().get("com.xiaoxiang.org.dao.Admin", id);
			return instance;
		} catch (RuntimeException re) {
	...
	public List findByExample(Admin instance) {
		...
		try {
			List results = getSession().createCriteria("com.xiaoxiang.org.dao.Admin").add(Example.create(instance))
					.list();
...
*/
	public void attachDirty(Admin instance) {
		try {
			...
			session=getSession();
			transation = session.beginTransaction();
			session.get(Admin.class, instance);
